...
#'
#' @export
load_cprdfiles <- function(pddir,dbf,ow=T,db=F,inctxt="TXTFILES",dropcodes=T){
  if(F){
    pddir <- cpath
    dbf <- sgdb
    ow=F
  ...
  names(cprdfiles) <- paste0("gold_",tolower(gsub("(^[0-9]*_|[.]txt)","",basename(cprdfiles))))
