...
#'
#' @export
load_therapy <- function(pddir,dbf,ow=F,db=F,tab_name="therapy",add=F,
    selvars2=c("patid","eventdate","sysdate","consid","prodcode","drugdmd","staffid","dosageid",
                  "bnfcode","qty","numdays","numpacks","packtype","issueseq","prn")){
  if(F){
    pddir <- gpath
    dbf <- sgdb
    ow <- F
  ...
  if(!tab_name%in%tabs || ow || add){
    difiles <- list.files(pddir,pattern="Therapy",full=T,recur=T)
