...
#'
#' @export
load_immunisation <- function(pddir,dbf,ow=F,db=F,tab_name="immunisation",add=F,
    selvars2=c("patid","eventdate","sysdate","constype","consid","medcode","sctid","sctdescid",
                 ...
                 "staffid","immstype","stage","status","compound","source","reason","method","batch")){
  if(F){
    pddir <- gpath
    dbf <- sgdb
    ow <- F
  ...
  if(!tab_name%in%tabs || ow || add){
    difiles <- list.files(pddir,pattern="Immunisation",full=T,recur=T)
